...
public class BotRunner {
    ...
    public static void main(String... args) throws Exception {
        ...
        Map<BotConfig.BotConfigName, BotConfig> configs = BotConfig.loadConfigs(props);
        if (configs.isEmpty()) {
    ...
    private BotRunner(Map<BotConfig.BotConfigName, BotConfig> configs) throws TelegramApiRequestException {
        ...
        registerTMDBCapability(configs.get(TMDB), capabilityList);
    ...
    private void registerTMDBCapability(BotConfig config, List<Capability> capabilityList) {
        if (config != null) {
            TMDBCapability tmdbCapability = new TMDBCapability(
                    new TMDBService(myAppModule.httpClient(), new TMDBService.TMDBConfig(config.getKey()))
            );
